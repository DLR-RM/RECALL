...
Code provided by Vincenco Lomonaco.

If you are using CORe50 in your publication please don't forget to also cite their paper:
Vincenzo Lomonaco and Davide Maltoni. "CORe50: a new Dataset and Benchmark for continual Object Recognition".
...
class CORE50(object):
    ...
    new2old_names = {'ni': 'ni', 'nc' : 'nc', 'multi-task-nc': 'nc', 'nic': 'nicv2_391', 'NC_cum' : 'nc', 'NC_inc' : 'nc', 'NIC_cum' : 'nicv2_2391', 'NIC_inc' : 'nicv2_391', 'NI_cum' : 'ni', 'NI_inc' : 'ni'}
    nbatch = {
        'ni': 8,
        'nc': 9,
        'nicv2_391': 391
    ...
    def __init__(self, root='', preload=True, scenario='nc', cumul=False,
        ...
        if sys.version_info >= (3, 0): #Python 3
            ...
            # dump pickle protokol to 2 (python 2 compatible):
            with open(os.path.join(self.root, "paths_v2.pkl"), 'wb') as f:
                pkl.dump(self.paths, f, protocol=2)
            with open(os.path.join(self.root, "LUP_v2.pkl"), 'wb') as f:
                pkl.dump(self.LUP, f, protocol=2)
            with open(os.path.join(self.root, "labels_v2.pkl"), 'wb') as f:
                pkl.dump(self.labels, f, protocol=2)

        else: #Python 2
            print("Using Python 2")
            print("Loading paths...")
            with open(os.path.join(self.root, 'paths_v2.pkl'), 'rb') as f:
                self.paths = pkl.load(f)

            print("Loading LUP...")
            with open(os.path.join(self.root, 'LUP_v2.pkl'), 'rb') as f:
                self.LUP = pkl.load(f)

            print("Loading labels...")
            with open(os.path.join(self.root, 'labels_v2.pkl'), 'rb') as f:
                self.labels = pkl.load(f)

        '''with open(os.path.join(root, "paths.txt"), 'w') as f:
            f.write("\n".join(self.paths))
        
        np.save(os.path.join(root, "LUP_new.npy"), np.array(self.LUP), fix_imports=True)
        np.save(os.path.join(root, "labels_new.npy"), np.array(self.labels), fix_imports=False)

        self.labels_2 = np.load(os.path.join(root, "labels_new.npy"), allow_pickle=True)
        with open(os.path.join(root, "paths.txt"), 'r') as f:
            self.paths_2 = f.read().split("\n")
        self.LUP_2 = np.load(os.path.join(root, "LUP_new.npy"), allow_pickle=True)
        
        for key in self.labels.keys():
            print(len(self.labels[key]))
            print(self.labels_2.shape)
            
            
            print("Loading of labels: {}".format(np.mean(self.labels_2[key] - self.labels[key])))
        for key in self.LUP.keys():
            print("Loading of LUP: {}".format(np.mean(self.LUP_2[key] - self.LUP[key])))    
    ...
    def __next__(self):
        ...
        if self.preload:
            train_x = np.take(self.x, train_idx_list, axis=0) \
                .astype(np.float32)
            train_paths = np.take(self.paths, train_idx_list, axis=0)
        else:
            print("Loading data...")
            train_root = os.path.join(self.root, "core50_128x128/")
            # Getting the actual paths
            train_paths = []
            for idx in train_idx_list:
                train_paths.append(os.path.join(train_root, self.paths[idx]))
            # loading imgs
        ...
        return train_x, train_y, train_paths, self.tasks_id[self.batch-1]
    ...
    def get_full_valid_set(self, reduced=True):
        ...
        if self.scenario == 'nc':
            ...
            val_root = os.path.join(self.root, "core50_128x128/")
            ...
            for idx, y in zip(valid_idx_list, valid_y):
                for i in range(self.nbatch[self.scenario]):
                    if y in self.labs_for_task[i]:
                        ...
                        y_x_task[i].append(y)
                        i_valid_paths[i].append(os.path.join(val_root, self.paths[idx]))
            ...
            for i in range(self.nbatch[self.scenario]):
                ...
                valid_set.append([(i_valid_x, i_valid_y, i_valid_paths[i]), i])
            ...
            if self.preload:
                ...
                valid_paths = np.take(self.paths, valid_idx_list, axis=0)
            else:
                # test paths
                valid_paths = []
                val_root = os.path.join(self.root, "core50_128x128/")
                print("valid_idx_list: {}".format(len(valid_idx_list)))
                for idx in valid_idx_list:
                    valid_paths.append(os.path.join(val_root, self.paths[idx]))
            ...
            for i in range(self.nbatch[self.scenario]):
                valid_set.append([(valid_x, valid_y, valid_paths), self.tasks_id[i]])
    ...
    def get_full_test_set(self):
        ...
        if self.scenario == 'nc':
            ...
            for i in range(self.nbatch[self.scenario]):
                ...
                full_test.append([(test_x, np.asarray([-1] * len(test_paths[i])), test_paths[i]), i])
            ...
            full_test = [[(test_x, np.asarray([-1] * len(test_paths)), test_paths), 0]]
